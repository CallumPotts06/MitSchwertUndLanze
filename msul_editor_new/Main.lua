...
shaders = require("EditorLibraries/EditorShaders")
...
MapEditor.Textures.Water = love.image.newImageData("MapTextures/BlueWaterTexture.png")
MapEditor.Textures.Sand = love.image.newImageData("MapTextures/SandTexture.png")
MapEditor.Textures.MudPath = love.image.newImageData("MapTextures/Path.png")
MapEditor.Textures.StonePath = love.image.newImageData("MapTextures/Path.png")
MapEditor.Textures.Road = love.image.newImageData("MapTextures/Road.png")
MapEditor.Textures.Farmland = love.image.newImageData("MapTextures/Farmland.png")

MapEditor.TextureKeys = {}
MapEditor.TextureKeys.Grass = 1
MapEditor.TextureKeys.Forest = 2
MapEditor.TextureKeys.Farmland = 3 
MapEditor.TextureKeys.MudPath = 4   
MapEditor.TextureKeys.StonePath = 5
MapEditor.TextureKeys.Road = 6
MapEditor.TextureKeys.Water = 7
MapEditor.TextureKeys.Sand = 8
...
function love.load()
    ...
    if LOAD_MODE == "emptymap" then
        ...
        for i=1,#tileList,1 do graphics.UpdateTileData(tileList[i]) end
    else
...
function love.draw()
    ...
    love.graphics.draw(TestBattalion,200,200,0,CameraZoom,CameraZoom)
end
...
function love.update(dt)
    ...
    for i = 1,#interactions,1 do
        ...
        if interactions[i].Name=="newForest" then
            currentMode = MapEditor.TextureKeys[ "Forest" ] EditorScreen.Screen.Visible = false  break
        elseif interactions[i].Name=="newGrass" then
            currentMode = MapEditor.TextureKeys[ "Grass" ] EditorScreen.Screen.Visible = false break

        elseif interactions[i].Name=="newWheat" then
            currentMode = MapEditor.TextureKeys[ "Farmland" ] EditorScreen.Screen.Visible = false break
        elseif interactions[i].Name=="newPotato" then
            currentMode = MapEditor.TextureKeys[ "MudPath" ] EditorScreen.Screen.Visible = false break


        elseif interactions[i].Name=="newPath" then
            currentMode = MapEditor.TextureKeys[ "StonePath" ] EditorScreen.Screen.Visible = false break
        elseif interactions[i].Name=="newRoad" then
            currentMode = MapEditor.TextureKeys[ "Road" ] EditorScreen.Screen.Visible = false break

        elseif interactions[i].Name=="newStream" then
            currentMode = MapEditor.TextureKeys[ "Water" ] EditorScreen.Screen.Visible = false break
        elseif interactions[i].Name=="newSand" then
            currentMode = MapEditor.TextureKeys[ "Sand" ] EditorScreen.Screen.Visible = false break
    ...
    if ( not mouseDown ) and ( #MapEditor.UpdatedTiles > 0 ) then
        for i = 1,#MapEditor.UpdatedTiles,1 do
            graphics.UpdateTileData(MapEditor.UpdatedTiles[i]) 
        end 
...
function getBrushIndexes(mousePos)
    ...
    if ( currentMode == 4 ) or ( currentMode == 5 ) or ( currentMode == 6 ) then
        currentBrushSize = ROAD_WIDTH
        if ( currentMode == 5 ) or ( currentMode == 6 ) then fillMode = "Square" end
    else
