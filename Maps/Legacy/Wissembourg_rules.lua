...
function maprules.LaunchBattle()
    ...
    for i=1,#GermanArmy,1 do
        for i2 = 1,#GermanArmy[i].Regiments,1 do
            table.insert(germanRegs, GermanArmy[i].Regiments[i2])
        end
    end

    local frenchRegs = {}
    for i=1,#FrenchArmy,1 do
        for i2 = 1,#FrenchArmy[i].Regiments,1 do
            table.insert(frenchRegs, FrenchArmy[i].Regiments[i2])
        end
    end

    BotPlayer.Init("France", "Easy", "Defensive", FrenchArmy, GermanArmy, "Right")
